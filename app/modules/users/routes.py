...
def users_list():
    ...
    users_data = []
    for u in users:
        # Get platforms via direct relationship
        user_platforms = []
    ...
    try:
        ...
        if removed_area_ids:
            for area_id in removed_area_ids:
                ...
                # Remove platforms from this area from user's platforms
                platforms_to_remove = [p for p in user.platforms if p.area_id == area_id]
                for p in platforms_to_remove:
                    user.platforms.remove(p)
                    add_audit_log(f"REVOCACIÓN: {user.email}", status="warning", detail=f"Acceso a {p.name} revocado por baja de área {area_obj.name}")
...
def delete_user(user_id):
    try:
        ...
        # Relationships are handled by backref or secondary table cleanup in SQLAlchemy
...
def user_access(user_id):
    try:
        ...
        approved_platform_ids = [p.id for p in user.platforms]
...
def update_user_access(user_id):
    try:
        ...
        # Sync platforms relationship
        selected_platforms = Platform.query.filter(Platform.id.in_(final_platform_ids)).all()
        user.platforms = selected_platforms
                
        db.session.commit()
        add_audit_log(f"ACTUALIZAR ACCESOS: {user.email}", status="success", detail=f"Accesos a plataformas actualizados")
    ...
    try:
        ...
        if removed_area_ids:
            for area_id in removed_area_ids:
                ...
                for p in platforms_to_remove:
                    user.platforms.remove(p)
                    add_audit_log(f"REVOCACIÓN ÁREA: {user.email}", status="warning", detail=f"Acceso a {p.name} revocado por baja de área {area_obj.name}")
